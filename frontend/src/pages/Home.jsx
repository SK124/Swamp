...
const Home = () => {
  ...
  useEffect(() => {
    const fetchSwamps = async () => {
      try {
        setLoading(true);
        const response = await fetch('http://localhost:8080/api/swamp?pageNumber=1&recordsPerPage=10');
            ...
            >
              <div className="flex space-x-4 w-max">
                {swamps && swamps.length > 0 ? (
                  swamps.map((swamp) => (
                    <Card key={swamp.ID} className="w-80 flex-shrink-0">
                      <CardHeader>
                      ...
                      <CardContent>
                        <div className="space-y-2">
                          ...
                          <div className="flex justify-between text-sm">
                            ...
                            <span>0/{swamp.MaxParticipants || "Unlimited"}</span>
                          </div>
