...
const getTopicName = (id) => MOCK_AVAILABLE_TOPICS.find(t => t.id === id)?.name || `Topic ID ${id}`;
// --- END MOCK DATA ---
...
const SwampDetail = () => {
  ...
  const handleJoinSwamp = () => {
    ...
    alert('Join Swamp functionality not implemented yet.');
  };
  ...
  const formattedStartTime = swampDetails.StartTime ? new Date(swampDetails.StartTime).toLocaleString([], {
        dateStyle: 'medium',
        timeStyle: 'short'
    }) : 'N/A';
      ...
      <Card className="w-full max-w-3xl mx-auto">
        <CardHeader>
          ...
          <CardDescription>
            ...
            Hosted by Owner ID: {swampDetails.OwnerID} | Starts: {formattedStartTime} | Duration: {swampDetails.Duration} mins
          </CardDescription>
          <div className="flex flex-wrap gap-2 pt-2">
           
            {swampDetails.Topics?.map(topicId => (
              <Badge key={topicId} variant="secondary">{getTopicName(topicId)}</Badge>
        ...
        <CardContent className="space-y-6">
          <div>
            ...
            <h3 className="text-lg font-semibold mb-2">Participants (0  / {swampDetails.MaxParticipants})</h3>
        ...
        <CardFooter className="flex flex-col sm:flex-row justify-between gap-2">
           ...
           <Button variant="destructive" onClick={handleLeaveSwamp}>Leave Swamp</Button>
           <Button onClick={handleJoinSwamp}>Join Swamp</Button>
           <Button variant="outline" onClick={handleGoToChat}>Go to Live Chat</Button>
        </CardFooter>
      </Card>
    </div>
  );
